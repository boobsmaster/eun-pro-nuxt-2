      ...
      if (value === '') {
        ...
        const filteredArray = this.containersList.map((element1) => {
          const filtered = element1.list.map((item) => {
            const filtered2 = item.list.filter((item) => item.name.includes(value))
  ...
  <div class="services">
    ...
    <div class="options">
      <OptionContainer
        ...
        :number="item.number"
      />
      <div v-if="isTouched && results.length === 0" class="options__placeholder">
        <span class="options__placeholder-text"> По вашему запросу не найдено результатов </span>
      </div>
